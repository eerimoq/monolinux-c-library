...
ML_UID(ml_one_wire_read_temperature_req);
ML_UID(ml_one_wire_read_temperature_rsp);
static ML_UID(read_temperature_timout);
...
static void ds18b20_finalize_busy(struct ml_queue_t *response_queue_p)
{
    struct ml_one_wire_read_temperature_rsp_t *response_p;

    response_p = ml_message_alloc(&ml_one_wire_read_temperature_rsp,
                                  sizeof(*response_p));
    response_p->res = -EBUSY;

    ml_queue_put(response_queue_p, response_p);
}
...
static void ds18b20_handle_read_temperature(
    ...
    struct ml_one_wire_read_temperature_req_t *request_p)
{
    ...
    if (self_p->ds18b20.response_queue_p != NULL) {
        ds18b20_finalize_busy(request_p->response_queue_p);
                                ...
                                struct cn_msg *cn_msg_p)
{
    ...
    while ((nl_msg_p = next_nl_msg(cn_msg_p, offset)) != NULL) {
        ...
        case W1_SLAVE_CMD:
            if (cn_msg_p->seq == (uint32_t)self_p->ds18b20.seqno) {
...
static int handle_socket(struct module_t *self_p)
{
    ...
    if (size < (ssize_t)sizeof(*message_p)) {
        ML_ERROR("Short data.");
...
static void *one_wire_main(struct module_t *self_p)
{
    ...
    while (res == 0) {
        ...
        if (event.data.fd == self_p->fd) {
            ...
            if (uid_p == &ml_one_wire_read_temperature_req) {
...
static int ds18b20_command_read(int argc, const char *argv[], FILE *fout_p)
{
    ...
    res = ml_one_wire_read_temperature(strtoull(argv[2], NULL, 16),
                                    &temperature);
...
void ml_one_wire_init(void)
{
    ...
    ml_queue_set_on_put(&module.queue,
                        ...
                        &module.put_fd);
    ml_queue_init(&module.read_temperature.response_queue, 2);
    pthread_mutex_init(&module.read_temperature.mutex, NULL);
...
int ml_one_wire_read_temperature(uint64_t slave_id, float *temperature_p)
{
    ...
    request_p = ml_message_alloc(&ml_one_wire_read_temperature_req,
                                 sizeof(*request_p));
    request_p->slave_id = slave_id;
    request_p->response_queue_p = &module.read_temperature.response_queue;

    pthread_mutex_lock(&module.read_temperature.mutex);

    ml_queue_put(&module.queue, request_p);
    ml_queue_get(&module.read_temperature.response_queue, &message_p);

    pthread_mutex_unlock(&module.read_temperature.mutex);

    response_p = (struct ml_one_wire_read_temperature_rsp_t *)message_p;
    res = response_p->res;
    *temperature_p = response_p->temperature;
